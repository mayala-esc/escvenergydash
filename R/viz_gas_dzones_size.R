...
highcharts_gas_mktsize <- function(
  ...
  title <-  paste0("Gas Offers Annual Bill - ", stringr::str_to_sentence(size), " retailers")
  
  subtitle <-  paste0("Median offers in ", stringr::str_to_sentence(dist_zone)," ", sign, " by $", s, " in ", month," ", year)
  
  caption <- "Source: Victorian Energy Compare. Note: light colour range denotes annual bills in the 10th to 90th percentile."
  ...
  highchart(type = "stock") %>%
    ...
    # custom exporting
    format_export(
      filename = paste("ESC_gas_offers", customer, dist_zone, size, sep = "_")
    ) %>%
...
#highcharts_gas_mktsize(size = "large")
